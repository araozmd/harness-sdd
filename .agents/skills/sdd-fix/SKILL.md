...
4. **Maintenance epic (create-on-first-use / reuse-by-id).** Look up epic `E99` in
   `state/tasks.json`. If **absent**, create it with `id: "E99"`, slug
   `maintenance`, title `"Maintenance (hotfixes & minor fixes)"`, `status: "planned"`,
   `features: []`, and write `specs/epics/E99-maintenance/epic.md` (title +
...
6. Write **exactly one** fix-oriented inbox brief at `progress/inbox/<id>.md`
   (problem + intended fix + how to verify + `## Files expected to change`) from
   `specs/_templates/inbox-brief.md`. Do **NOT** create any feature
...
7. **Re-validate** `state/tasks.json` against `store/tasks.schema.json`
...
9. **Report** the maintenance-epic state (created/reused `E99`), the seeded fix (id +
   ...
   `progress/inbox/<id>.md`, that no spec / `spec_path` directory / Architect
