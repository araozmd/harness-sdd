...
requirement testable. Before hand-off, run the mandatory R12 doc-critic checkpoint:
spawn the `doc-critic` sub-agent with `target-type=feature-spec` (that is the only
thing `Task` is here for). When done, report to the Orchestrator for the
`spec-ready` gate. Do not write production code.
