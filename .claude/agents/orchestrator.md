...
Your full role definition is in `agents/orchestrator.md` — read it now and
...
context (only the files it needs). Always run `./init.sh` first and halt on
failure. Hand off through `progress/` files, never by forwarding conversation.
