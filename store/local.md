...
- **The two positional arguments are the contract, and `sync-board.mjs` honors them**
  (E99-F156). The bundled mirror reconciles **only the named object** when both are
  present, so one status write costs one object's worth of API calls instead of the whole
  board's. An **epic id** is legitimate here — "the id selects the object kind" above —
  and what it does depends on the provider's object model: under **github-projects** epics
  are a single-select *field*, not an issue, so a targeted epic id is a clean **no-op, exit
  0**; under **jira** an epic *is* an issue, so a targeted epic id syncs **that epic**.
  Under both, an id matching nothing is an **error**, not a fallback to a full reconcile.
  Invoked with **no** positionals it still reconciles board-wide, so a by-hand full sync is
  unchanged.
