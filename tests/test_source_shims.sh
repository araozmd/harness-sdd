...
def m_codex_toml():
    import tomllib
    raw = read(os.path.join(SRC, ".codex", "agents", ROLE + ".toml"))
    if raw is None:
        return ABSENT
    try:
        ...
        return ABSENT
    prompt = cfg.get("developer_instructions", "")
    return Rec(present=True, name=cfg.get("name", ""), targets=role_refs(prompt),
               harness_rooted=".harness/" in prompt, mode="subagent",
               described=bool(cfg.get("description", "")))
