...
When explicitly invoked — as `$sdd-fix` (Codex's spelling) or `/sdd-fix` (Antigravity's) — treat all text accompanying that mention as the value of `$ARGUMENTS` in the canonical instructions below.
