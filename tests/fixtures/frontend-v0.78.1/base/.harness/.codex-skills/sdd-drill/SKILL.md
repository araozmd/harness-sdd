...
When explicitly invoked — as `$sdd-drill` (Codex's spelling) or `/sdd-drill` (Antigravity's) — treat all text accompanying that mention as the value of `$ARGUMENTS` in the canonical instructions below.
