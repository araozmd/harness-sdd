...
When explicitly invoked — as `$sdd-pr-loop` (Codex's spelling) or `/sdd-pr-loop` (Antigravity's) — treat all text accompanying that mention as the value of `$ARGUMENTS` in the canonical instructions below.
