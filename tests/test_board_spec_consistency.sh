...
set_status() { HARNESS_DIR="$FX" python3 "$FX/tools/tasks-lock.py" set-status "$@" >"$T/sl.out" 2>"$T/sl.err"; }
...
set_status E1-F1 done --evidence "none: frontmatter-sync fixture, no work to land" || fail "R23: set-status failed: $(cat "$T/sl.err")"
