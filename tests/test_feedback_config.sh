...
test_version_and_changelog() {
  [ "$(cat "$SRC/VERSION")" = "0.88.0" ] || fail "R11: VERSION is not 0.88.0 (got $(cat "$SRC/VERSION"))"
  ...
  pass "VERSION is 0.88.0 and CHANGELOG's [0.85.0] section names the block, its default-on enabled: true, the default repo, the notice, and the opt-out (R11) [test_version_and_changelog]"
}
