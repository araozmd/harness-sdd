...
#   - PROJECT-authored content (specs/product.md, specs/glossary.md, state/tasks.json,
#     specs/epics, progress) is seeded once on a fresh install and NEVER clobbered on
#     upgrade.
...
EOF
  ...
  # harness body. Three states, one rule, no flag (docs/RATIONALE.md ablation doctrine):
  # absent -> seed the shipped example; byte-identical to the shipped example -> pristine,
  # leave it identical; anything else -> preserve, byte for byte. The ONE exception is the
  # existing umbrella-stub sentinel (a pre-E30-F01 thin child left one at this path) — that
  # is harness-written, not project content, and is re-materialised from the shipped
  # example. Branch order is load-bearing: `-L` before `-e`, because `-e` dereferences a
  # symlink and would let a dangling link read as "absent" and get written THROUGH. The
  # pristine test runs before the stub test so a shipped example that ever began with the
  # sentinel could not be misread. `$SRC` is the running installer's own source — the same
  # reference `copy()` uses — never the umbrella body (D1: this path left the prose tier
  # entirely; see harness-install.sh's HARNESS_BODY_PROSE comment above and docs/UMBRELLA.md).
...
BODY LAYOUT  (E24-F03 + E24-F04 / ADR-0004):
  ...
  thin  the PROSE tier is pointer stubs resolved from umbrella.root; the PROGRAM tier
    ...
    prose (stub-able) : AGENTS.md agents/ docs/ specs/_templates/
...
PROJECT-OWNED  (seeded once, never clobbered on upgrade):
  ...
  .harness/specs/product.md  .harness/specs/glossary.md  .harness/specs/epics/
...
6. **Greenfield branch only — Write** `.harness/specs/architecture.md` from
   ...
   `.harness/specs/_templates/adr.md` (4-digit, above the max existing ADR number); the
...
7. **Repo topology output.** Repo topology is an output of planning,
   ...
   `.harness/specs/architecture.md` names more than one deployable, write exactly
   one `repo-topology ADR` at `.harness/specs/adr/NNNN-<title>.md` — a single decision
   ...
   `.harness/specs/architecture.md`, probing the complete set of keys already assigned —
   ...
   On a greenfield run, when `.harness/specs/architecture.md` names exactly one deployable
   ...
   **complete replacement snapshot**: it names the **full** effective deployable set as it
   ...
   Planner additionally removes the derived `.harness/umbrella.manifest.draft.yaml`, while
...
9. **Doc-critic checkpoint (before re-validation).** Spawn the **Doc-critic**
   ...
   no changes outside the appended section: the committed `.harness/specs/vision.md`,
   the rest of `.harness/specs/architecture.md`, and every existing ADR stay untouched,
...
11. **Report** the artifacts written (`.harness/specs/vision.md`,
   `.harness/specs/architecture.md`, each `.harness/specs/adr/NNNN-*.md`, and, when
   the plan names multiple deployables, the draft manifest at
   `.harness/umbrella.manifest.draft.yaml`), the seeded
...
4. **Mint the session token once, then call the reporter** with the allow-listed upstream
   ...
   if [ -z "$_hf_session" ] && [ -f .harness/telemetry.jsonl ]; then
     _hf_session="$(grep -E '"type"[[:space:]]*:[[:space:]]*"session-start"' .harness/telemetry.jsonl 2>/dev/null \
       | tail -n 1 \
...
if [ -z "$default_branch" ] || [ -z "$base_ref" ] || [ "$base_ref" != "$default_branch" ]; then
  ...
  gh pr edit "$pr_number" --add-label needs-human >/dev/null 2>&1 || true
elif ! sh .harness/tools/wait-for-codex.sh merge-verify "$round_dir" "$pr_number" pre; then
  ...
  msg=".harness/.pr-loop/$pr_number/squash-message.txt"
  if [ -s "$msg" ]; then
    gh pr merge "$pr_number" --squash --delete-branch --body-file "$msg" --match-head-commit "$reviewed_head" && merge_oid="$(sh .harness/tools/wait-for-codex.sh merge-verify "$round_dir" "$pr_number" post)" && merged=1
  else                        # no message composed — squash with GitHub's default body
    gh pr merge "$pr_number" --squash --delete-branch --match-head-commit "$reviewed_head" && merge_oid="$(sh .harness/tools/wait-for-codex.sh merge-verify "$round_dir" "$pr_number" post)" && merged=1
  fi
else
  gh pr merge "$pr_number" --merge --delete-branch --match-head-commit "$reviewed_head" && merge_oid="$(sh .harness/tools/wait-for-codex.sh merge-verify "$round_dir" "$pr_number" post)" && merged=1
fi
```
...
# Reconcile either the source artifacts or only the final arming verdict through
# the same pristine/symlink guards and manifest writer.
self_reconcile() {
  python3 - "$SRC" "$1" "${2:-artifacts}" <<'PYSELF'
import os, pathlib, subprocess, sys
root, staged = map(pathlib.Path, sys.argv[1:3])
arming_only = sys.argv[3] == "arming"
...
for rel in expected | prior.keys():
    if arming_only != (str(rel) == ".escalation-arming"): continue
    key=pathlib.Path(*rel.parts[:3]) if str(rel).startswith('.agents/skills/') else rel
    units.setdefault(key,set()).add(rel)
# Preserve complementary, still-proven entries until their own reconciliation.
# In particular, never publish the temporary install's optimistic arming verdict.
owned=[rel for rel in prior if arming_only != (str(rel) == ".escalation-arming")
       and safe(rel) and (root/rel).is_file() and checksum(root/rel)==prior[rel]]
...
"
  ...
  self_reconcile "$_si_tmp/source"

  # The temporary install proves what we intended to write. Arming must describe
  # the actual reconciled source Builder roles, including protected collisions.
  # Feed that observation to the existing consumer verdict writer; do not infer
  ...
  for _si_fe in $SELECTED; do
    ...
    for _si_role in builder builder-heavy; do
      ...
      if [ -L "$SRC/.$_si_fe" ] || [ -L "$SRC/.$_si_fe/agents" ] \
         || [ -L "$SRC/$_si_rel" ] || [ ! -f "$SRC/$_si_rel" ] \
         || ! cmp -s "$SRC/$_si_rel" "$_si_tmp/source/$_si_rel" \
         || ! grep -qxF "$(CDPATH= cd -- "$SRC" && cksum "$_si_rel" 2>/dev/null)" "$SRC/.claude/.glue-manifest"; then
        mark_unstamped "$_si_fe"
      fi
    done
  done
  write_escalation_arming
  rm -f "$_UNSTAMPED_FILE"
  if [ -f "$H/.escalation-arming" ]; then
    cp "$H/.escalation-arming" "$_si_tmp/source/.escalation-arming"
  else
    rm -f "$_si_tmp/source/.escalation-arming"
  fi
