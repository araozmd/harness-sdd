...
EOF
  ok "Claude Code agents + /sdd-next + /sdd-new + /sdd-plan + /sdd-drill installed (.claude/)"

  # ── 5b. OpenCode commands (regenerated each run) ────────────────────────────
  # Claude Code reads .claude/commands/; OpenCode reads .opencode/command/. Mirror
  # the just-written command bodies there so /sdd-next, /sdd-new, /sdd-plan and
  # /sdd-drill show up in OpenCode too. With no `agent:` frontmatter the command runs
  ...
  ok "OpenCode commands /sdd-next + /sdd-new + /sdd-plan + /sdd-drill installed (.opencode/)"
