...
- [ ] **T1** (R1) — In `harness-install.sh` §4 (`write_pointer` / `GEMINI.md`), confirm
...
- [ ] **T2** (R2,R4,R6) — In `install_one()`, immediately after the §5b OpenCode block
...
- [ ] **T3** (R2,R3) — In §5c, write `$TARGET/.agent/rules/harness.md`: a short rule that
...
- [ ] **T4** (R4,R5) — In §5c, loop over `orchestrator architect builder reviewer scout`
...
- [ ] **T5** (R6,R7,R8,R9) — In §5c, loop over `sdd-next sdd-new sdd-plan sdd-drill
...
- [ ] **T6** (R12) — In §5c, add an `ok "Antigravity glue (rules + agents + workflows)
  installed (.agent/)"` line for parity with the existing front-end `ok` lines.

- [ ] **T7** (R10) — In §3, extend the `manifest.txt` HARNESS-OWNED list to mention
  `.agent/rules/*  .agent/agents/*  .agent/workflows/*  (repo root, regenerated)`.

- [ ] **T8** (R10) — Bump `VERSION` from `0.20.0` to `0.21.0` (MINOR — new
  backward-compatible capability).

- [ ] **T9** (R10) — Add a `## [0.21.0]` section to `CHANGELOG.md` under
...
- [ ] **T10** (R11) — In `tests/test_install.sh`, after the OpenCode assertion group,
...
- [ ] **T11** — Run the full `verification.test_command` suite (`sh tests/test_install.sh
