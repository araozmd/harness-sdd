...
- [ ] **T1** (R1, R2, R3) — Edit `store/tasks.schema.json`: add `"draft"` and
  ...
  `planned`.
- [ ] **T2** (R4) — Edit `init.sh`: in the embedded fallback validator, change
  `EPIC_STATUS = {"pending", "in-progress", "done"}` to also include `"draft"`
  and `"planned"`.
- [ ] **T3** (R12) — Edit `init.sh`: immediately after `data` is loaded (before the
...
- [ ] **T4** (R5, R6, R8) — Edit `agents/orchestrator.md` step 3 ("Read state"):
...
- [ ] **T5** (R5, R6, R7, R11) — Edit `store/local.md`: (a) extend the **next()**
...
- [ ] **T6** (R9) — Edit `docs/WORKFLOW.md`: add a short "Epic lifecycle" section
...
- [ ] **T7** (R10) — Edit `specs/_templates/epic.md`: update the frontmatter status
  comment to `# draft → planned → in-progress → done (pending = legacy alias of
  planned; rollup of its features)`.
- [ ] **T8** (R13) — Edit `store/board-mirror.md`: add a one-paragraph note that
  ...
  entries.
- [ ] **T9** (R1–R13) — Create `tests/test_epic_lifecycle.sh` per
...
- [ ] **T11** (R14) — Bump `VERSION` by one MINOR (current value + MINOR — do not
  ...
  updates.
- [ ] **T12** — Run `./init.sh` and the full `verification.test_command`; ensure
