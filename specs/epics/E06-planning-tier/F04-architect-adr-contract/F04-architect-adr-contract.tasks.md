...
- [x] **T1** (R1, R2, R3, R4, R5, R7, R8, R9, R10, R18) — Amend `agents/architect.md`,
  ...
  - State the contract is portable (lives in the role file, runs on any AGENTS.md CLI)
    (R18).
- [x] **T2** (R6) — Edit `specs/_templates/feature.spec.md`: add a `## Architecture
...
- [x] **T3** (R11, R12) — Amend `agents/reviewer.md` with a **new, additive** section
...
- [x] **T4** (R13) — Edit `docs/SPEC-FORMAT.md`: document the `## Architecture alignment`
...
- [x] **T5** (R14) — Edit `docs/WORKFLOW.md`: add a **new** `## Architecture-aligned specs
...
- [x] **T6** (R15) — Edit `README.md`: one-line note that feature specs cite the
  architecture decisions (ADRs) they touch, beside the existing SDD-contract / `/sdd-*`
  mentions.
- [x] **T7** (R1–R19) — Create `tests/test_architect_adr.sh` per
...
- [x] **T8** (wiring) — Edit `harness.config.yaml`: append `&& sh
  tests/test_architect_adr.sh` to `verification.test_command` and extend its trailing
  comment (e.g. `+ architect-adr`).
- [x] **T9** (R19) — Bump `VERSION` by one MINOR (read the current value first; do not
...
- [x] **T10** — Run `./init.sh` and the full `verification.test_command`; ensure green
