...
1. Run `./init.sh` from the project root. If it exits non-zero, STOP: the harness
   considers this environment broken and the probe must not write a capability marker.
2. Prepare a temp directory under `progress/opencode-concurrency-probe/`
...
6. Write the result to `.opencode-parallel` as exactly one word:
