...
`feedback.*` (`harness.config.yaml`, E32-F01) is **project** layer: one committed,
per-repo decision, never personal. In an umbrella it is **not inherited** — each child's
own `feedback:` block governs sessions rooted in that child, and the coordinator's block
neither overrides nor is overridden by it, the same standalone-tier rule the rest of
`harness.config.yaml` already follows.
