...
# 2za. Self-generated glue staleness (E26-F02) — SOURCE layout only. The harness source
#      repo's `.claude/` glue is generated by `harness-install.sh --self`, which records
#      a cksum ledger in `.claude/.glue-manifest`. A hand edit to a generated file (or a
#      deleted one) is surfaced here as ONE warn-only line naming the remedy. Targets
#      never carry harness-install.sh or the manifest (HARNESS_BODY_LOCAL ships
#      neither), so this is silent everywhere but the source checkout — and silent when
#      everything matches. Warn-only by contract: staleness must never block an agent.
GLUE_MANIFEST="$HARNESS_DIR/.claude/.glue-manifest"
if [ -f "$HARNESS_DIR/harness-install.sh" ] && [ -f "$GLUE_MANIFEST" ]; then
  GLUE_STALE=""
  while IFS= read -r _gm_line; do
    [ -n "$_gm_line" ] || continue
    _gm_path="$(printf '%s\n' "$_gm_line" | awk '{print $3}')"
    [ -n "$_gm_path" ] || continue
    if [ "$(cd "$HARNESS_DIR" && cksum "$_gm_path" 2>/dev/null)" != "$_gm_line" ]; then
      GLUE_STALE="$GLUE_STALE $_gm_path"
    fi
  done < "$GLUE_MANIFEST"
  if [ -n "$GLUE_STALE" ]; then
    echo "⚠️  generated glue diverges from its --self manifest:$GLUE_STALE — hand edits are clobbered by regeneration; run 'sh harness-install.sh --self' and commit (warn-only)"
  fi
fi
