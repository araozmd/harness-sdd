...
# (Codex #160 P2). Report files (.info/.html/.json/.xml/.txt directly under coverage/)
# and the lcov-report/ + tmp/ subtrees are output; source files under coverage/ count.
#
...
GEN_RE='(^|/)(vendor|node_modules|dist|build)/|(^|/)coverage/(lcov-report|tmp)/|(^|/)coverage/[^/]+\.(info|html|json|xml|txt)$|\.lock$|(^|/)(package-lock\.json|yarn\.lock|poetry\.lock|Cargo\.lock|go\.sum)$|\.(pb|generated)\.[a-z]+$|\.mutbak$'
