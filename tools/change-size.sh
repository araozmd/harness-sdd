...
# `coverage/` is generated output in effectively every JS/Python ecosystem, and a repo
# that emits it into a non-ignored dir otherwise books ~20k phantom "production" lines —
# an E99-F24 round 1 reported ESCALATE on exactly that and a Reviewer spent real effort
# disproving it. One alternation, near-universal, same instrument as vendor/dist.
#
...
GEN_RE='(^|/)(vendor|node_modules|dist|build|coverage)/|\.lock$|(^|/)(package-lock\.json|yarn\.lock|poetry\.lock|Cargo\.lock|go\.sum)$|\.(pb|generated)\.[a-z]+$|\.mutbak$'
