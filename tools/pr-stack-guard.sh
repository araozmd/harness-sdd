...
#
# The stacked-PR lane lets a safely-splittable feature ship reviewably: increment B is
# opened against increment A's branch instead of the default branch, so the reviewer reads
# only B's own diff. Stacking provides incremental **review**, not atomic delivery — merging
# increment A publishes wave 1 to the default branch while B and C are still open.
#
